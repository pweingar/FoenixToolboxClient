...
class Function:
	...
	def emit_latex(self, output):
		"""Emit the description of the function in LaTeX."""
...
def process_definition(type, name, parameters, comments, prototype):
	...
	for comment in comments:
		...
		if m2:
			...
			m3 = re.match("@brief\s+(.*)$", comment)
			if m3:
				func.set_brief(m3.group(1).strip())
			else:
				m4 = re.match("@return\s+(.*)$", comment)
				if m4:
					func.set_return_value(m4.group(1).strip())
				else:
...
#
with open("bindings.tex", "w") as bindings:
	for func in functions:
		func.emit_latex(bindings)
