...
size_t _Stub_write(int fd, const void *buf, size_t count) {
	init();
	if (file_descriptor[fd].is_open) {
		short n = sys_chan_write(/* file_descriptor[fd].toolbox_fd */ 0, (unsigned char *)buf, (short)count);
		return n;
	} else {
		return -1;
	}
